...
if (slugParts.length === 1) {
  const toolName = slugParts[0] as keyof typeof linux;
  const found = linux[toolName];
  ...
  const category = slugParts[0] as keyof typeof linux;
  const toolName: string = slugParts[1] as string;

  if (linux[category] && typeof linux[category] === 'object') {
    tool = (linux[category] as any)[toolName];
  }
}
